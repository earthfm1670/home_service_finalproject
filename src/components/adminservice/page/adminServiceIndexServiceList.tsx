...
function IconTrash({ id, setServicesList, index, serviceName }: IconTrashProp) {
  // interface Service {
  //   service_id: number;
  // }
  ...
  const handleDelete = async (serviceId: number) => {
    try {
      ...
      if (response.status === 201) {
        ...
        setServicesList(
          (prevTable) => prevTable.filter((_, idx: number) => idx !== index)
...
function IconEdit({ id }: IconEditProp) {
  // console.log(id)
