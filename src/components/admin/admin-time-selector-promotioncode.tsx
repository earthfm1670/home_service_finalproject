...
export const TimeSelectorAdminPromotionCode: React.FC<TimeSelectorProps> = ({
  value,
  onChange,
  selectedDate,
}) => {
  ...
  const isTimeDisabled = (hour: string, minute: string) => {
    ...
    // ตั้งเวลาใหม่จากวันที่ที่เลือก
    const selectedDateTime = new Date(selectedDate);
    selectedDateTime.setHours(parseInt(hour, 10), parseInt(minute, 10), 0, 0);
  
    // ใช้เวลาปัจจุบันจาก state
    const now = currentDateTime; 
    const oneHourLater = new Date(now.getTime() + 60 * 60 * 1000);
  
    // ตรวจสอบว่าเวลาที่เลือกในวันนี้มันน้อยกว่าหนึ่งชั่วโมงจากเวลาปัจจุบันหรือไม่
    if (selectedDate.toDateString() === now.toDateString()) {
      return selectedDateTime < oneHourLater; // ปิดการเลือกเวลาเมื่อเวลาน้อยกว่า 1 ชั่วโมงจากตอนนี้
    }
  
    // สำหรับวันที่ในอนาคต สามารถเลือกเวลาได้ทุกเวลา
    return false;
  };
